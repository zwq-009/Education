		...
		<view class="shipinpart">
			<view class="shipinpart-media">
				...
				<view v-else-if="videostate === 'audio'" class="audio">
					<image :src="menuinfo.thumb?(HOST_URL + menuinfo.thumb):'' " mode="aspectFill" :class="gaosi?'audio_bg gaosi_filter':'audio_bg' "></image>
					<view class="bofang" @click="play">
						<image :src="audio_paused?'../../static/play-big.png':'../../static/pause1.png'" mode=""></image>
					</view>
			...
			<view class="shipinpart-info">
				<text class="info-span1">{{menuinfo.menuname}}</text>
				...
				<view class="info-span5">
					<text class="info-span3">
						<text>{{coursehour}}课时</text>
						<text>|</text>
						<text>{{menuinfo.viewnum ? menuinfo.viewnum : '0'}}人学过</text>
					</text>
					<text class="info-span4" v-if="action=='' ">￥{{menuinfo.price}}</text>
	...
	export default {
		...
		onLoad(e) {
			...
			if(e.action){
				...
				if(e.action=='credit'){
					this.get_creditinfo(e.menuid,'course')
				}else if(e.action=='seckill'){
					this.get_seckillinfo(e.menuid,'course')
				}else if(e.action=='pintuan'){
					this.get_pintuaninfo(e.menuid,'course')
				}
			}
			getCourseDetails({menuid:e.menuid,uid:this.uid}).then(res => {
				...
				this.coursehour = this.courses.length
				if(this.menuinfo.price==0 || this.menuinfo.is_pay==1){
					this.is_free=true
				}
				// #ifdef H5
				...
				if (this.$jwx && this.$jwx.isWechat()) {
					this.$jwx.share({
						...
						'desc': menuinfo.jianjie,
						'link': link,
						'img_url': HOST_URL+menuinfo.thumb,
					})
					this.$jwx.sharepyq({
						'title': menuinfo.menuname,
						'link': link,
